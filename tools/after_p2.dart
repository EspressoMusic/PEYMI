...
class _BakeryHomePageState extends State<BakeryHomePage> {
  ...
  final List<Map<String, String>> _products = const [
    {'name': 'חלת שבת', 'price': '22₪', 'category': 'לחמים'},
    {'name': 'עוגת שוקולד ביתית', 'price': '58₪', 'category': 'עוגות'},
    {'name': 'מארז קראנצ\' קוקיז', 'price': '34₪', 'category': 'עוגיות'},
    {'name': 'בורקס גבינה', 'price': '18₪', 'category': 'מלוחים'},
    {'name': 'מגש מאפי בוקר', 'price': '75₪', 'category': 'מארזים'},
  ];

  final List<Map<String, String>> _deals = const [
    {'title': '1+1 על עוגיות הבית', 'desc': 'בקנייה מעל 80₪', 'valid': 'עד יום שישי'},
    {'title': '20% הנחה על מגשי אירוח', 'desc': 'בהזמנה מוקדמת של יומיים', 'valid': 'השבוע בלבד'},
    {'title': 'משלוח חינם', 'desc': 'בהזמנה מעל 150₪', 'valid': 'כל החודש'},
  ];
  ...
  Widget build(BuildContext context) {
    final List<Widget> pages = [
      _DealsPage(deals: _deals),
      _OrdersPage(orders: _orders),
      const _SettingsHelpPage(),
      _CatalogPage(products: _products),
    ];
...
class _DealsPage extends StatelessWidget {
  const _DealsPage({required this.deals});
...
class _OrdersPage extends StatelessWidget {
  const _OrdersPage({required this.orders});
